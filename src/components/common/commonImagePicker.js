				...
				<Animated.View style={[{flex: 1,backgroundColor: mainBackgroundColor},this.props.style]}>
					<TouchableOpacity style={{flex: 1}} activeOpacity={1} onPress={()=>{
						...
						<Animated.View style={[styles.buttonsView,{bottom: buttonsViewMarginTop}]}>
							<Button activeOpacity={0.7} style={styles.buttons}
								...
								onPress={()=>{
									...
									ImagePicker.openCamera({
									  width: configData.width || 300,
									  height: configData.height || 400,
									  cropping: configData.cropping || true,
									  includeBase64: true,
									}).then(image => {
							...
							<Button activeOpacity={0.7} style={styles.buttons}
								...
								onPress={()=>{
									...
									ImagePicker.openPicker({
										...
										if (cancleAction) {cancleAction()}
										let targetImageArr = images
										targetImageArr = images.map((item,index)=>{
											const hash =  item.localIdentifier.split('/')[0];
									    item.uri = 'assets-library://asset/asset.JPG?id=' + hash + '&ext=JPG';
											return item
										})
							...
							<View style={{height: 10,width,backgroundColor: 'lightgray'}}></View>
							<Button activeOpacity={0.7} style={styles.buttons}
...
const styles = StyleSheet.create({
	...
	buttons: {
		...
		marginBottom:0,
		borderBottomWidth: 1,
		borderBottomColor: 'gray'//COLOR.LINE_COLOR
	},
	buttonText: {
		color: 'blue',//COLOR.APP_THEME,
		fontSize: 15
	}
})
